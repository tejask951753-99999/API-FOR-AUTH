...
def invite_reseller(data: ResellerInviteRequest):
    ...
    if not frontend_origin:
        frontend_origin = data.frontend_origin.rstrip('/') if data.frontend_origin else "https://authfusion.top"
        if "localhost" in frontend_origin or "127.0.0.1" in frontend_origin:
            frontend_origin = "https://authfusion.top"
    else:
...
def list_resellers(data: ResellerListRequest):
    ...
    for r in resellers:
        ...
        if not app_doc:
            continue
        app_name = app_doc[0].to_dict().get('name', '')
        if not app_name:
            continue
